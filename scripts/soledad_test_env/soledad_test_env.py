...
def status_from_pidfile(args, default_tempdir):
    tempdir = _get_tempdir(args, default_tempdir)
    pidfile = os.path.join(tempdir, args.pidfile)
    try:
        pid = get_pid(pidfile)
        psutil.Process(pid)
...
COUCH_TEMPDIR = '/tmp/couch_test'


def _get_tempdir(args, default):
...
def couch_server_start(args):
    tempdir = _get_tempdir(args, COUCH_TEMPDIR)
    pidfile = os.path.join(tempdir, args.pidfile)
    logfile = os.path.join(tempdir, args.logfile)
    ...
    if not config_file:
        config_file = tempfile.mktemp(prefix='couch_config_', dir=tempdir)
        lines = []
        with open(args.template) as f:
            lines = f.readlines()
            lines = map(lambda l: l.replace('BASEDIR', tempdir), lines)
...
def couch_status_from_pidfile(args):
    status_from_pidfile(args, COUCH_TEMPDIR)
...
SOLEDAD_SERVER_TEMPDIR = '/tmp/soledad_server_test'
...
SOLEDAD_SERVER_URL = 'http://127.0.0.1:2424'

SOLEDAD_CLIENT_PASS = '12345678'
SOLEDAD_CLIENT_TEMPDIR = '/tmp/soledad_client_test'
SOLEDAD_CLIENT_UUID = '1234567890abcdef'


def soledad_server_start(args):
    tempdir = _get_tempdir(args, SOLEDAD_SERVER_TEMPDIR)
    pidfile = os.path.join(tempdir, args.pidfile)
    logfile = os.path.join(tempdir, args.logfile)
    private_key = os.path.join(tempdir, args.private_key)
    cert_key = os.path.join(tempdir, args.cert_key)
...
def soledad_server_status_from_pidfile(args):
    status_from_pidfile(args, SOLEDAD_SERVER_TEMPDIR)
...
CERT_CONFIG_FILE = './conf/cert_default.conf'
...
def cert_create(args):
    call([
        ...
        '-keyout', args.private_key,
        '-out', args.cert_key])


def cert_delete(args):
    os.unlink(args.private_key)
    os.unlink(args.cert_key)
...
def run_cli():
    ...
    cmd_sol_server_start.add_arguments([
        ...
        (['--tempdir', '-t'], {'default': SOLEDAD_SERVER_TEMPDIR}),
    ])
    ...
    cmd_sol_client_test.add_arguments([
        ...
        (['--basedir', '-b'], {'default': SOLEDAD_CLIENT_TEMPDIR}),
        (['--server_url', '-s'], {'default': SOLEDAD_SERVER_URL}),
        (['--cert_file', '-C'], {'default': SOLEDAD_SERVER_CERTKEY}),
        (['--auth-token', '-a'], {'default': SOLEDAD_SERVER_AUTH_TOKEN}),
    ])
